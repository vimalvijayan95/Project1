...
#1.Definition of the number of neurons within layer and total interactions 
num_hidden_neurons = [90]
num_iter = 100
...
learning_rate = np.linspace(1e-5,1e-2,100)
...
#1.Deinition of the number of neurons within layer and total interactions 
...
#2.Construction of the deep neural network with a given activation function.
 ...
 #with tf.variable_scope('dnn', reuse=True):
    ...
    #b)Hidden layer
    for l in range(1):
        current_layer = tf.layers.dense(previous_layer, neur_number ,activation=tf.nn.sigmoid)
        previous_layer = current_layer
