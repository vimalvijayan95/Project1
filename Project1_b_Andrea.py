...
#alphav=np.linspace(1e-3,100,100)
nalpha = 500
alphav = np.logspace(-3, 5, nalpha)
np.random.seed(3155)
...
#In this case we fit with Ridge for EACH value of alpha
 ...
 #plt.scatter(alphav, mean_mse_cv[:,degree-2], label = 'cross_val_score')
 plt.scatter(np.log10(alphav), mean_mse_cv[:,degree-2], label = 'cross_val_score')
